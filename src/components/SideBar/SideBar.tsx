...
const SideBar = () => {
  const [value, setValue] = useState<{ id: number; expanded: boolean }>({
    id: 1,
    expanded: false,
  });
  ...
  return (
    ...
    >
      <div className="text-2xl bg-lime-400 grid place-items-center max-w-[50px] p-2  aspect-square rounded-md text-primaryWhite font-semibold tracking-wider m-2">
        <FaBucket className="text-green-900 " />
      </div>
      <div className="h-full">
        ...
        <div className="menu-items flex flex-col gap-2 my-2">
          <p className="flex items-center select-none gap-2 p-2 px-4 rounded-md transition-all hover:bg-lime-200 cursor-pointer font-medium">
                ...
                return item.subMenu.length > 0 ? (
                  <>
                    <li
                      ...
                      className="flex capitalize justify-between select-none items-center gap-2 p-2 px-4 rounded-md transition-all hover:bg-lime-200 cursor-pointer font-medium"
                    >
                    ...
                    <ul className="relative">
                      {value.id == item.id &&
                        ...
                        item.subMenu.map((data: Item, index) => {
                          return (
                            <li
                              className="capitalize p-2 ml-4 pl-4 border-gray-300 border-l-2 before:absolute before:left-0 before:w-3 before:translate-y-3 before:translate-x-4 before:h-[2px] before:rounded-md before:bg-gray-300 hover:bg-lime-200 rounded-sm transition-all cursor-pointer font-semibold"
                              key={index}
                            >
                              {data.name}
                            </li>
